...
Usage: `python extract_submissions.py <input_zip> <output_folder> --ignore <file1,file2>`
...
## `browse_images.py`
Usage: `python browse_images.py <submissions_dir> --student <student>`

This tool allows the user to browse all submitted images. Unlike `grade_images.py`, no reference images are required.
Optionally, `--student <student>` can be passed in to start at a particular student name or index.
Controls:
- comma/period: previous/next image.
- n/m: previous/next student.
- o: open the folder containing the student's images in the file browser.
...
## `initialize_projects.py`
Usage: `python initialize_projects.py <starter_code_dir> <submissions_dir> <output_dir> --files <file1,file2> --student <student> --silent`

Initialize projects for each student. For each student, a copy of the starter code is made, and then the specified `--files` are copied into the starter code.